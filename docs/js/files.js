...
function popup_open(item, video = '') {
	...
	if (curent_popup && unlock) {
		...
		if (!document.querySelector('.header._active')) {
			body_lock_add(500);
		}
...
function popup_close(item, bodyUnlock = true) {
	if (unlock) {
		...
		if (!document.querySelector('.header._active') && bodyUnlock) {
...
document.addEventListener('DOMContentLoaded', function () {
	let evenSlides = document.querySelectorAll('.chose-slider__product:nth-child(even)');
	let oddSlides = document.querySelectorAll('.chose-slider__product:nth-child(odd)');
	processSlides(evenSlides);
	processSlides(oddSlides);
});
function processSlides(slides) {
	if (!slides) return;
	let slidesHeights = [];
	for (const slide of slides) {
		let height = parseInt(getComputedStyle(slide).getPropertyValue('height'));
		slidesHeights.push(height);
	}
	let maxHeight = Math.max(...slidesHeights);
	for (const slide of slides) {
...
let slider1 = new Swiper('.chose-slider__body', {
	speed: 800,
	...
	// Arrows
	navigation: {
		prevEl: '#chose-slider-arrow-prev',
		nextEl: '#chose-slider-arrow-next',
	},
});

let slider2 = new Swiper('.combo-slider__body', {
	speed: 800,
	watchSlidesVisibility: true,
	breakpoints: {
		320: {
